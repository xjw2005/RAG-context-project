...
def group_sentences_by_breakpoints(
    ...
    for index, sentence in enumerate(sentences):
        ...
        if should_split:
            chunks.append(" ".join(current_chunk).s))
            current_chunk = []
            current_length = 0
...
def _fallback_character_split(text: str, chunk_size: int, chunk_overlap: int) -> List[str]:
    ...
    if chunk_size <= 0:
        raise ValueError("chunk_st be greater than 0")
...
class SemanticAwareChunker(TextChunker):
    ...
    def chunk(
        ...
        if len(sentence_embeddings) != len(sentences):
            return group_sentences_by_breakpoints(sentences, [], max_chunk_size)     # 计算相邻句子之间的余弦相似度，用于判断语义连贯性
        similarities = [
