...
class VectorStore:
    ...
    def add_text(self, chunk_id: str, text: str) -> None:
        """添加文本块（不包含向量）"""
        self._doc_map.append((chunk_id, text, "text", None))
    ...
    def add_vector(self, vector_id: str, embedding: np.ndarray, metadata: Dict[str, Any]) -> None:
        ...
        # 添加到映射
        self._doc_map.append((vector_id, metadata.get("text", ""), "vector", metadata))
    ...
    def get_chunk(self, idx: int) -> Tuple[str, str, Any, Any]:
        """根据索引获取切块
            Args:
                idx (int): 索引
            Returns:
